...
> 搭项目、配工具链、发版与治理依赖时加载(refactor skill 的配套工程篇)。跨语言通用；各语言的具体工具命令、配置/锁文件名、pre-commit 命令见 `languages/{lang}.md` 的基线(§0)与工具链节，日志写法见其「安全与日志」节。
...
- **配置与锁文件入库**：项目的配置文件与锁文件必须提交，保证依赖可复现；具体文件名清单见各语言规范的基线/工具链相关章节。
...
**跨语言工具链对照表**（概览；具体命令/配置文件名/flag 见各语言规范的基线/工具链相关章节）：
