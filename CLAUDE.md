...
本文件用于指导 AI 助手在本仓库中编写代码时应遵循的规范。规范分为四个领域：**技术栈与工具基线**（用什么）、**编码实践**（怎么写）、**项目与工程化**（项目怎么搭）、**版本与协作**（怎么协作发布）。

## 一、技术栈与工具基线

### 1. 优先复用成熟的开源组件

**核心原则**：能用成熟、维护良好的开源库解决的问题，就不要自己手写。这样可以减少冗余代码、降低出错概率，并提升可读性与可维护性。
...
| 场景 | 推荐库 | 引入要求 | 说明 |
| --- | --- | --- | --- |
| 路径操作 | [`pathe`](https://github.com/unjs/pathe) | 必须 | 跨平台一致的路径处理，替代 Node 内置 `path`，避免 Windows/POSIX 分隔符差异。 |
| 错误处理 | [`neverthrow`](https://github.com/supermacro/neverthrow) | 必须 | 使用 `Result` 类型显式处理错误，避免随意 `throw` 异常导致的隐式控制流。 |
| 日期 / 时间 | [`Temporal`](https://github.com/tc39/proposal-temporal) | 必须 | 使用 Temporal API 处理日期与时间，替代易错的原生 `Date`。 |
| 运行时类型校验 / Schema | [`zod`](https://github.com/colinhacks/zod) | 必须 | 定义 Schema 并校验外部输入（API、表单、配置），同时推导 TS 类型。 |
| HTTP 请求 | [`ofetch`](https://github.com/unjs/ofetch) | 按需 | 对 `fetch` 的增强封装，自动解析、错误处理、重试，替代手写 `fetch` 样板。 |
| 命令行接口 | [`commander`](https://github.com/tj/commander.js) / [`citty`](https://github.com/unjs/citty) | 必须 | 声明式构建 CLI，替代手写 `process.argv` 解析；通用场景选 `commander`（生态大），unjs/极简场景选 `citty`。 |
| 环境变量 | [`dotenv`](https://github.com/motdotla/dotenv) | 按需 | 从 `.env` 加载环境变量，配合 `zod` 做校验。 |
| 工具函数 | [`es-toolkit`](https://github.com/toss/es-toolkit) | 按需 | 现代化的工具函数集合（替代 lodash），体积更小、原生 TS。 |
| 测试 | [`vitest`](https://github.com/vitest-dev/vitest) | 必须 | 快速、原生支持 ESM/TS 的测试框架。 |
| 日志 | [`pino`](https://github.com/pinojs/pino) | 按需 | 高性能结构化 JSON 日志。 |
| 唯一 ID | [`nanoid`](https://github.com/ai/nanoid) | 按需 | 轻量、安全的唯一 ID 生成；简单场景可直接用 `crypto.randomUUID()`。 |
| 数据库 / ORM | [`drizzle-orm`](https://github.com/drizzle-team/drizzle-orm) | 必须 | TypeScript 原生、类型安全的 SQL ORM，替代手写 SQL 拼接。 |
| 文件 glob 匹配 | [`tinyglobby`](https://github.com/SuperchupuDev/tinyglobby) | 按需 | 轻量、快速的文件匹配，替代体量更大的 `glob`/`fast-glob`。 |
| Lint / 格式化 | [`biome`](https://github.com/biomejs/biome) | 必须 | 一体化的 Lint + 格式化工具，追求开箱即用/速度时优先；需要丰富插件规则时仍可用 `eslint`(flat config) + `prettier`。 |
| 子进程 / 命令执行 | [`execa`](https://github.com/sindresorhus/execa) | 按需 | 更友好、安全的子进程调用，替代原生 `child_process`。 |
| 终端美化 / Spinner | [`picocolors`](https://github.com/alexeyraspopov/picocolors) / [`ora`](https://github.com/sindresorhus/ora) | 按需 | 轻量终端着色与加载动画，提升 CLI 体验。 |
| 异步并发控制 | [`p-limit`](https://github.com/sindresorhus/p-limit) | 按需 | 限制 Promise 并发数，替代手写并发队列。 |

#### Python

| 场景 | 推荐库 | 引入要求 | 说明 |
| --- | --- | --- | --- |
| 路径操作 | [`pathlib`](https://docs.python.org/3/library/pathlib.html) | 必须 | 使用面向对象的 `Path`，替代基于字符串拼接的 `os.path`。 |
| 命令行接口 | [`typer`](https://github.com/fastapi/typer) | 必须 | 基于类型注解构建 CLI，替代手写 `argparse`。 |
| 数据校验 / 模型 | [`pydantic`](https://github.com/pydantic/pydantic) | 必须 | 基于类型注解做数据校验与序列化，替代手写校验逻辑。 |
| 配置管理 | [`pydantic-settings`](https://github.com/pydantic/pydantic-settings) | 必须 | 从环境变量 / `.env` 加载并校验配置，替代手写 `os.getenv`。 |
| HTTP 请求 | [`httpx`](https://github.com/encode/httpx) | 必须 | 同步 / 异步统一的 HTTP 客户端，替代 `requests` + `aiohttp`。 |
| 日期 / 时间 | [`pendulum`](https://github.com/python-pendulum/pendulum) | 按需 | 时区友好的日期时间处理；简单场景可用标准库 `datetime` + `zoneinfo`。 |
| 终端输出 / 富文本 | [`rich`](https://github.com/Textualize/rich) | 按需 | 彩色输出、表格、进度条、日志美化。 |
| 日志 | [`loguru`](https://github.com/Delgan/loguru) | 按需 | 开箱即用的结构化日志，替代繁琐的 `logging` 配置。 |
| 测试 | [`pytest`](https://github.com/pytest-dev/pytest) | 必须 | 简洁的测试框架，配合 fixture 与参数化。 |
| 重试 | [`tenacity`](https://github.com/jd/tenacity) | 必须 | 声明式重试逻辑（退避、超时、条件），替代手写循环。 |
| Web / API 框架 | [`fastapi`](https://github.com/fastapi/fastapi) | 必须 | 基于类型注解与 `pydantic` 的高性能异步 API 框架。 |
| 包 / 环境管理 | [`uv`](https://github.com/astral-sh/uv) | 必须 | 极快的依赖与虚拟环境管理，替代 `pip` + `venv`。 |
| 代码格式化 / Lint | [`ruff`](https://github.com/astral-sh/ruff) | 必须 | 极快的 Lint + 格式化工具，替代 `flake8` + `black` + `isort`。 |
| 数据库 / ORM | [`sqlalchemy`](https://github.com/sqlalchemy/sqlalchemy) / [`sqlmodel`](https://github.com/fastapi/sqlmodel) | 必须 | 复杂查询/生产首选 `sqlalchemy` 2.0；与 FastAPI 快速搭配、需类型安全模型时用 `sqlmodel`（结合 `pydantic`）。 |
| 静态类型检查 | [`mypy`](https://github.com/python/mypy) / [`pyright`](https://github.com/microsoft/pyright) | 必须 | 基于类型注解做静态检查，提前发现类型错误。 |
| 数据处理 / DataFrame | [`polars`](https://github.com/pola-rs/polars) | 按需 | 新项目/大数据性能优先选 `polars`；需兼容既有绘图/ML 等 `pandas` 生态时用 `pandas`。 |
| 任务队列 / 后台任务 | [`celery`](https://github.com/celery/celery) / [`arq`](https://github.com/python-arq/arq) | 必须 | 分布式任务队列；`arq` 为轻量 asyncio 方案。 |
| 子进程 / 命令执行 | 标准库 [`subprocess`](https://docs.python.org/3/library/subprocess.html) | 标准库 | 优先用 `subprocess.run`，跨平台可靠；避免 `sh` 等不支持 Windows 的库。 |
| 序列化 / 高性能 JSON | [`orjson`](https://github.com/ijl/orjson) | 按需 | 极快的 JSON 序列化/反序列化；仅在性能敏感时引入，否则用标准库 `json`。 |
...
| 用途 | 工具 | 说明 |
| --- | --- | --- |
| 包管理 | [`pnpm`](https://github.com/pnpm/pnpm) | 快速、节省磁盘，原生支持 workspace/monorepo；提交 `pnpm-lock.yaml`。 |
| Lint + 格式化 | [`biome`](https://github.com/biomejs/biome) | 一体化、极快；替代 `eslint` + `prettier`（需丰富插件时再退回该组合）。 |
| 类型检查 | `tsc --noEmit` | 以 TypeScript 编译器做类型校验，`strict: true`。 |
| 测试 | [`vitest`](https://github.com/vitest-dev/vitest) | 原生 ESM/TS、快速，统一单测与覆盖率。 |
| 构建 / 打包 | [`tsup`](https://github.com/egoist/tsup) / [`tsdown`](https://github.com/rolldown/tsdown) | 零配置打包库；应用按框架自带构建（Vite 等）。 |
| 直接运行 TS | [`tsx`](https://github.com/privatenumber/tsx) | 免编译直接执行 `.ts` 脚本。 |

#### Python

| 用途 | 工具 | 说明 |
| --- | --- | --- |
| 包 / 环境 / 版本管理 | [`uv`](https://github.com/astral-sh/uv) | 统一管理依赖、虚拟环境与 Python 版本；提交 `uv.lock`。 |
| Lint + 格式化 | [`ruff`](https://github.com/astral-sh/ruff) | 一体化、极快；替代 `flake8` + `black` + `isort`（`ruff check` + `ruff format`）。 |
| 类型检查 | [`mypy`](https://github.com/python/mypy) / [`pyright`](https://github.com/microsoft/pyright) | 静态类型检查，开启 strict 模式。 |
| 测试 | [`pytest`](https://github.com/pytest-dev/pytest) | 统一测试框架，配合覆盖率（`pytest-cov`）。 |
...
- **代码自解释**：优先用清晰命名与小函数表达意图，减少「为了解释烂代码而写的注释」。
...
- **优先纯函数**：核心逻辑写成无副作用的纯函数，把 I/O、随机、时间等副作用推到边界，提升可测性。
...
**核心原则**：先测量再优化；只优化有实质影响的热点，不牺牲可读性做过早优化。

- **先测量再优化**：用数据（profiler / benchmark）定位真实瓶颈，禁止凭直觉做「过早优化」。
- **优化关键路径**：只优化对用户体验或成本有实质影响的热点；非热点优先保持可读性。
- **基准可复现**：关键算法/接口写 benchmark（JS 用 `vitest bench`，Python 用 `pytest-benchmark`），优化前后对比，纳入回归。
- **避免常见浪费**：N+1 查询、循环内重复计算/IO、不必要的深拷贝、未加索引的查询；批量与缓存优先于逐条处理。
- **并发与异步**：IO 密集用异步并发（见本域「异步与并发」），CPU 密集考虑 worker / 多进程；不阻塞事件循环 / 主线程。
- **缓存有度**：缓存须明确失效策略与一致性边界，避免引入难调试的陈旧数据问题。
...
- **测试可读**：测试代码同样遵循命名与风格规范，避免过度抽象导致难以理解失败原因。
