...
- 版本号唯一来源：JS 看 `package.json`、Python 看 `pyproject.toml`，并与 Git tag（`vX.Y.Z`）一致。
...
- **锁文件必须提交**：JS `pnpm-lock.yaml`、Python `uv.lock`；CI 安装校验一致性（`pnpm install --frozen-lockfile`、`uv sync --locked`），禁静默更新。
...
- **CI 强制扫描**：每次 PR 与主分支扫漏洞，达阈值即阻断合并。JS/TS 用 `pnpm audit --audit-level=high`（或 `osv-scanner`），Python 用 [`pip-audit`](https://github.com/pypa/pip-audit)（或 `osv-scanner`）。
...
- **依赖精简**：定期清理未使用依赖（`knip`/`depcheck`、`deptry`）；引入新依赖前对照「优先复用成熟的开源组件」的判定。
...
- **必跑检查清单**：安装（锁文件校验）→ 格式校验 → Lint → 类型检查 → 测试（带覆盖率）→ 构建 → 漏洞扫描；任一失败即阻断。
- **与本地一致**：CI 命令与本地、pre-commit 一致（见主规范「统一工具链」），避免「本地过、CI 挂」。
