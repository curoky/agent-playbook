...
**核心原则**：先读懂项目既有设计与约定，再动手；设计一旦变更，同步回写文档。

- **动手前加载**：若存在 `README.md` 或其他设计/约定文档（如 `docs/`、`design.md`、`ARCHITECTURE.md`、`ADR`），且其中含项目的设计决策或约定，每次改动前先把相关文档读入上下文，按其约束执行，不另起一套。
- **变更即回写**：交互过程中若设计有调整、约定有变更（如接口契约、目录结构、技术选型、命名规则变化），按情况同步更新这些文档，保持文档与代码一致；不静默偏离已记录的约定。
- **冲突先澄清**：代码现状与文档约定冲突，或需偏离既有设计时，先指出并与用户确认，再决定改代码还是改文档（与「思考在先」呼应）。
