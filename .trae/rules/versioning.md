...
**核心原则**：提交遵循 [Conventional Commits](https://www.conventionalcommits.org/)，使历史可读、可机器解析，并驱动版本与 changelog。
...
- **锁文件必须提交**：JS `pnpm-lock.yaml`、Python `uv.lock`、Go `go.mod` + `go.sum`、C++ `MODULE.bazel` + `MODULE.bazel.lock`；CI 校验一致性（`pnpm install --frozen-lockfile`、`uv sync --locked`、`go mod verify` + `go mod tidy` 后无 diff、`bazel mod deps --lockfile_mode=error`），禁静默更新。
