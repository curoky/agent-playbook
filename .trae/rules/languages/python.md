...
- 新项目用官方最新稳定 Python（截至 2026-07 为 3.14.x）；`pyproject.toml` 写 `requires-python = ">=3.14"`，`.python-version` 锁最新 patch（当前 3.14.6）。用 `uv` 管理环境与依赖，提交 `.python-version`、`uv.lock`。
...
- IO 密集用 `asyncio`/线程；CPU 密集默认 `ProcessPoolExecutor`，隔离解释器适用时可用 3.14 的 `InterpreterPoolExecutor`。free-threaded CPython 仅在依赖兼容且 benchmark 证明有收益时采用。
...
| ASGI 运行 | [`uvicorn`](https://github.com/Kludex/uvicorn) / [`granian`](https://github.com/emmett-framework/granian) | 默认用 `uvicorn`，多进程直接用 `uvicorn --workers`；现有 Gunicorn 部署用独立 [`uvicorn-worker`](https://github.com/Kludex/uvicorn-worker)，不用已废弃的 `uvicorn.workers`；需要 HTTP/2 或 benchmark 证明有收益时评估 `granian`。 |
...
- pre-commit 用 [`lefthook`](https://github.com/evilmartians/lefthook) 对暂存 Python 文件跑 `ruff check` 与 `ruff format --check`；CI 跑全项目 `ruff check`、`ruff format --check`、strict `mypy`/`pyright` 与 `pytest`。
